...
class GameObject:
    ...
    def __init__(self, coordinate,speed,direction, parent_surface):
    ...
    def draw_object(self):
        self.parent_surface.blit(self.surface, self.coordinate)
    ...
    def shoot(self):
        if time.time()-self.last_shoot_timestamp>self.kd:
...
class Tank:
    ...
    def move(self, direction, list_of_objects):
        self.player_rect = pygame.Rect(self.coordinate, [32, 32])
        res = self.player_rect.collidelistall(list_of_objects)
        if not res:
            self.prev_coordinates = copy.deepcopy(self.coordinate)
            self.coordinate[0] += self.speed * direction[0]
            self.coordinate[1] += self.speed * direction[1]
        else:
            self.coordinate = self.prev_coordinates
