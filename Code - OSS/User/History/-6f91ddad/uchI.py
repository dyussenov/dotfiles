...
enemies = [
    GameObject([256, 64], 5, [1, 0], sc),
    GameObject([512, 512], 5, [0, 1], sc),
    GameObject([640, 512], 5, [1, 0], sc)
]
...
while 1:
    ...
    if keys[pygame.K_SPACE] and time.time()-kd > 1.5:
        bullet_cord = [player.coordinate[0],player.coordinate[1]]
        kd = time.time()
    ...
    for e in enemies:
        ...
        e.draw_object()
        if not e.shoot_status:
            bullets.append(Bullet(copy.deepcopy(e.coordinate), copy.deepcopy(e.direction), pygame.image.load('textures/bullet.png')))
            e.shoot_status = True

    for b in bullets:
        res = pygame.Rect(b.coordinate, (16,16)).collidelistall(list_of_objects)
        if res:
            bullets.remove(b)
            continue

        b.move()
        b.check_collision([player], kill_sound) #here
        b.blit_bullet(sc)
    ...
    if player.coordinate[0] < 0 or player.coordinate[1] < 0:
        if not alarm.status:
