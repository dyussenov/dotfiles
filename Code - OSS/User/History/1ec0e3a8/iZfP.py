...
def group_download(request):
    ...
    writer.writerow(['Список групп', datetime.date.today()])
    writer.writerow(keys)
    writer.writerows([i.values() for i in groups])
    return response
...
def students_download(request):
    ...
    writer = csv.writer(response)
    writer.writerow(['Список студентов', datetime.date.today()])
    writer.writerow(keys)
    writer.writerows([i.values() for i in students])
    return response

@login_required_decorator
def student_get_doc(request, pk):
    students = services.get_student_by_id(pk)
    print(students)
    return render(request, 'spravka.html', {'name': students[0]['first_name'], 'surname': students[0]['last)name']})