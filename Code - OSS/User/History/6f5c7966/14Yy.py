...
class AuthService:
    ...
    async def register_new_user(self, user_data: models.User) -> models.Token:      
        password_hash = self.hash_password(user_data.password)
        print(password_hash)
        setattr(user_data, 'password_hash', password_hash)
    ...
    async def find_user(self, login) :#-> models.User
        a= await users_collection.find_one({'login': login})
        print(a)
        return 123
