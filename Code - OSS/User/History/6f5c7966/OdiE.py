...
class AuthService:
    ...
    async def update_user(self, user_data: models.User, update_data: dict) -> models.Token:
        #print(update_data.__dict__)
        print(user_data)
        updating = deepcopy(user_data)
        for k, v in update_data.__dict__.items():
            if v:
                updating.__setattr__(k, v)
        await users_collection.update_one({'login': 'qwerty1229'}, {'$set': {'description':'ccuumm'}})
        return 200
